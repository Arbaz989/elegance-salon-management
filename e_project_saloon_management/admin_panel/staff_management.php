...
if ($user_role !== 'admin') {
    echo "Only admins can manage staff profiles.";
    exit();
}

// Handle form submission for updating staff profiles
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $staff_id = $_POST['staff_id'];
    $contact_info = $_POST['contact_info'];
    $commission_rate = $_POST['commission_rate'];

    $sql = "UPDATE users SET contact_info = '$contact_info', commission_rate = '$commission_rate' WHERE id = '$staff_id'";
    
    if ($conn->query($sql) === TRUE) {
        $success_message = "Staff profile updated successfully!";
    } else {
        $error_message = "Error updating staff profile: " . $conn->error;
    }
}

// Fetch all stylists and receptionists with their current contact info and commission rates
$staff_result = $conn->query("SELECT id, user_name, contact_info, commission_rate, role FROM users WHERE role IN ('stylist', 'receptionist')");
...
<title>Manage Staff Profiles</title>
    <style>
        ...
        .container { max-width: 800px; margin: auto; padding: 20px; background-color: #fff; border: 1px solid #ccc; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.1); }
        ...
        form input[type="text"], form input[type="number"] { width: calc(100% - 10px); padding: 5px; margin-top: 5px; }
        form button { background-color: #4CAF50; color: white; border: none; padding: 10px 20px; cursor: pointer; }
...
<head>
    ...
    <title>Sloon Management - Tables</title>
                ...
                <tr>
                    ...
                    <td>
                        <form method="POST">
                            ...
                            <input type="text" id="contact_info" name="contact_info" value="<?= $row['contact_info'] ?>" required>
                            <label for="commission_rate">Commission Rate (%):</label>
                            <input type="number" id="commission_rate" name="commission_rate" value="<?= $row['commission_rate'] ?>" required>
